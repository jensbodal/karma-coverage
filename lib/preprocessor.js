...
function createCoveragePreprocessor (logger, helper, basePath, reporters, coverageReporter) {
  ...
  return function (content, file, done) {
    ...
    if (instrumenterLiteral === 'istanbul') {
      var functionName = InstrumenterConstructor.prototype.functionName;
      overwriteFunctionName(functionName);

      function overwriteFunctionName(superMethod) {
        InstrumenterConstructor.prototype.functionName = function(node, line, location) {
          var id = superMethod.call(this, node, line, location);

          this.coverState.fnMap[id].decl = this.coverState.fnMap[id].loc;
